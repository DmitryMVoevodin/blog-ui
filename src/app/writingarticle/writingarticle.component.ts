...
@Component({
  selector: 'app-writingarticle',
  templateUrl: './writingarticle.component.html',
  styleUrls: ['./writingarticle.component.css']
})
export class WritingArticleComponent implements OnInit {
  ...
  private forMeOrForCreator:string;
  ...
  ngOnInit() {
    ...
    this.articleService.castArticleSelectedForWriting.subscribe(articleSelectedForWriting => this.selectedArticle = articleSelectedForWriting);
    ...
    if(this.user.userId == this.selectedArticle.articleCreator.userId) {
      this.forMeOrForCreator = 'Для меня';
    } else {
      this.forMeOrForCreator = 'Для автора статьи'
    }
  ...
  public writeArticle(): void {

    this.selectedArticle.articleRights = this.keyStringForRights;
    this.articleService.writeArticle(this.selectedArticle, this.user.userId).subscribe(
      result => {
        if (result) {

          this.articleService.deleteSettingsForOnePersonInGroup(
            this.selectedArticle.articleTopic.topicId, this.selectedArticle.articleId).subscribe(
            resultDeleting => {
              if (resultDeleting) {
                ...
                for(var iUser = 0; iUser < this.allUsers.length; iUser++) {
                  ...
                  if (isNeededToBeAddedToDataBase) {
                    ...
                    this.articleService.addSettingsForOnePersonInGroup(
                      this.selectedArticle.articleTopic.topicId, this.selectedArticle.articleId, nicknameAndRights).subscribe(
            ...
            errorDeleting => {alert('An unexpected error occurred while deleting previous settings');}
          );
